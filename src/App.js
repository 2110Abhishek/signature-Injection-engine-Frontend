...
const BACKEND_BASE_URL =
  ...
  "https://signature-injection-engine-backend-iosb.onrender.com";
...
function App() {
  ...
  const computePixelsFromNormalized = (coordinate) => {
    if (!coordinate || !pagePixelSize.width || !pagePixelSize.height) return null;
  ...
  const handleMouseMove = (e) => {
    ...
    if (dragState) {
      ...
      const found = fields.find((f) => f.id === dragState.fieldId);
      if (!found) return;
      const width = computePixelsFromNormalized(found.coordinate).width;
      const height = computePixelsFromNormalized(found.coordinate).height;

      const normalized = computeNormalizedFromPixels(newLeft, newTop, width, height);
      if (!normalized) return;
      setFields((prev) => prev.map((f) => (f.id === dragState.fieldId ? { ...f, coordinate: normalized } : f)));
    }
    ...
    if (resizeState) {
      ...
      if (!normalized) return;
      setFields((prev) => prev.map((f) => (f.id === resizeState.fieldId ? { ...f, coordinate: normalized } : f)));
    }
  };
  ...
  const handlePdfFileChange = async (e) => {
    ...
    try {
      ...
      const finalUrl = pdfUrl && pdfUrl.startsWith("http") ? pdfUrl : `${BACKEND_BASE_URL}${pdfUrl}`;
  ...
  const updateFieldChecked = (id, checked) => {
    setFields((prev) => prev.map((f) => (f.id === id ? { ...f, checked } : f)));
  };
  ...
  const handleSignDocument = async () => {
    try {
      ...
      const signedUrl =
        res.data.signedPdfUrl && res.data.signedPdfUrl.startsWith("http")
          ? res.data.signedPdfUrl
          : `${BACKEND_BASE_URL}${res.data.signedPdfUrl}`;
  ...
  const viewerWidthStyle = viewMode === "mobile" ? { maxWidth: "420px", width: "100%" } : { maxWidth: "900px", width: "100%" };

  const fieldsForCurrentPage = fields.filter((f) => f.pageIndex === currentPage - 1);
  const selectedField = fields.find((f) => f.id === selectedFieldId) || null;

  return (
    <motion.div className="app-root" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      <motion.header className="header" initial={{ y: -50 }} animate={{ y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <motion.div className="header-badge" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: "spring" }}>
          <CheckCircle size={14} />
      ...
      <div className="layout">
        <motion.aside className="sidebar" initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }}>
          <motion.div className="sidebar-section" whileHover={{ scale: 1.02 }}>
            ...
            <div className="upload-area">
              <label className="upload-label">
                ...
                <input type="file" accept="application/pdf" onChange={handlePdfFileChange} className="upload-input" />
              </label>
              {pdfMeta.pdfId && (
                <motion.div className="upload-info" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  <div className="upload-info-content">
                    <span className="upload-status">Loaded</span>
                    <span className="upload-id">{String(pdfMeta.pdfId).slice(0, 8)}...</span>
                  </div>
          ...
          <motion.div className="sidebar-section" whileHover={{ scale: 1.02 }}>
            ...
            <div className="signature-upload">
              <label className="signature-label">
                ...
                <input type="file" accept="image/png,image/jpeg" onChange={handleSignatureFileChange} className="signature-input" />
              </label>
              {signatureFile && (
                <motion.div className="signature-preview" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <motion.div className="sidebar-section" whileHover={{ scale: 1.02 }}>
            ...
            <div className="field-editor">
              <AnimatePresence>
                {selectedField ? (
                  <motion.div className="field-editor-content" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
                    <div className="field-header">
                      <div
                        className="field-type-badge"
                        style={{
                          background: (FIELD_TYPES.find((f) => f.type === selectedField.type)?.color || "#000") + "20",
                    ...
                    <div className="field-properties">
                      ...
                      {(selectedField.type === "text" || selectedField.type === "date") && (
                        <div className="property">
                          ...
                          <input type="text" value={selectedField.value || ""} onChange={(e) => updateFieldValue(selectedField.id, e.target.value)} className="property-input" placeholder="Enter value..." />
                        </div>
                      )}
                      ...
                      {selectedField.type === "radio" && (
                        <div className="property checkbox-property">
                          <label className="checkbox-label">
                            <input type="checkbox" checked={!!selectedField.checked} onChange={(e) => updateFieldChecked(selectedField.id, e.target.checked)} className="checkbox-input" />
                  ...
                  <motion.div className="field-editor-empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <main className="viewer-container">
          <motion.div className="viewer-toolbar" initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
            <div className="toolbar-section">
              <div className="view-mode-buttons">
                <button onClick={() => setViewMode("desktop")} className={`view-mode-btn ${viewMode === "desktop" ? "active" : ""}`}>
                  <Monitor size={16} />
                  <span>Desktop</span>
                </button>
                <button onClick={() => setViewMode("mobile")} className={`view-mode-btn ${viewMode === "mobile" ? "active" : ""}`}>
                  <Smartphone size={16} />
                  <span>Mobile</span>
                </button>
              </div>
            ...
            <div className="toolbar-section">
              <div className="zoom-controls">
                <button onClick={() => setScale((prev) => Math.max(0.5, prev - 0.1))} className="zoom-btn">
                  <ZoomOut size={16} />
                </button>
                <div className="zoom-display">
                  <span>{Math.round(scale * 100)}%</span>
                </div>
                <button onClick={() => setScale((prev) => Math.min(2, prev + 0.1))} className="zoom-btn">
                  <ZoomIn size={16} />
                </button>
              </div>
            </div>
            ...
            <div className="toolbar-section">
              <div className="page-controls">
                <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={!numPages || currentPage <= 1} className="page-btn">
                  <ChevronLeft size={16} />
                </button>
                ...
                <button onClick={() => setCurrentPage((p) => (numPages ? Math.min(numPages, p + 1) : p))} disabled={!numPages || currentPage >= numPages} className="page-btn">
            ...
            <div className="toolbar-section">
              <motion.button className="sign-button" onClick={handleSignDocument} disabled={isSigning || isLoadingPdf || !pdfMeta.url} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                {isSigning ? (
                  <>
          ...
          <div className="pdf-viewer">
            <motion.div className="pdf-container" style={viewerWidthStyle} ref={pageContainerRef} onDragOver={handleOverlayDragOver} onDrop={handleOverlayDrop} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.3 }}>
              {pdfMeta.url ? (
                <div className="pdf-document">
                  <Document file={pdfMeta.url} onLoadSuccess={handleDocumentLoadSuccess} loading={<div className="pdf-loading"><div className="loading-spinner" /><span>Loading PDF...</span></div>} onLoadError={(err) => { console.error(err); alert("Failed to load PDF"); }}>
                    <Page pageNumber={currentPage} scale={scale} onRenderSuccess={handlePageRenderSuccess} renderTextLayer={false} renderAnnotationLayer={false} />
                  </Document>
                        ...
                        >
                          <div className="field-content">
                            ...
                            <span className="field-label">
                              {field.type === "radio" ? (field.checked ? "✓ Radio" : "○ Radio") : field.type}
                            </span>
                          </div>
                          <div className="resize-handle" onMouseDown={(e) => handleResizeMouseDown(e, field.id)}>
                            <Maximize2 size={8} />
                          </div>
                        </motion.div>
                      );
                ...
                <motion.div className="pdf-empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <motion.footer className="footer" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }}>
        ...
        <div className="footer-right">
          <span className="security-badge">🔒 SHA-256 • MongoDB Audit Trail • Secure Processing</span>
        </div>
      </motion.footer>

      <AnimatePresence>
        {showFieldPreview && previewField && (
          <motion.div className="field-preview-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="field-preview" style={{ color: previewField.color }}>
              {previewField.icon && <previewField.icon size={24} />}
